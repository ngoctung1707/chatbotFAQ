...
export default async function Page({ params }: Props) {
  ...
  const { docs, totalPages, nextPage, prevPage } = await payload.find({
    ...
    where: {
      tag: {
        in: ['software-engineering-and-decentralized-systems'],
      },
      lang: {
        equals: lang,
      },
                            ...
                            <ul className="pagination list-wrap">
                              {getPaginationGroup.length <= 0 ? null : (
                                <li className="next_link page-item">
                                  {currentPage === 1 ? null : (
                                    <Link
                                      href={`/research/r&d-labs/software-engineering-and-decentralized-systems/pages/${prevPage}`}
                                    >
                              ...
                              {getPaginationGroup.map((item, index) => {
                                return (
                                  ...
                                  >
                                    <Link
                                      href={`/research/r&d-labs/software-engineering-and-decentralized-systems/pages/${item}`}
                                    >
                              ...
                              {getPaginationGroup.length <= 0 ? null : (
                                <li className="next_link page-item">
                                  {currentPage >= totalPages ? null : (
                                    <Link
                                      href={`/research/r&d-labs/software-engineering-and-decentralized-systems/pages/${nextPage}`}
                                    >
                                      <span className="page-link">
