...
export default async function Page({ params }: Props) {
  ...
  const { docs, totalPages, nextPage, prevPage } = await payload.find({
    ...
    where: {
      tag: {
        in: ['operational-efficiency-in-finance'],
      },
      lang: {
        equals: lang,
      },
              ...
              <div className="row justify-content-center">
                <div className="col-xl-12">
                  <div className="section-title text-center mb-40 tg-heading-subheading animation-style3">
                    <h2 className="title tg-element-title">Operational Efficiency in Finance</h2>
                  </div>
                </div>
              </div>
                            ...
                            <ul className="pagination list-wrap">
                              {getPaginationGroup.length <= 0 ? null : (
                                <li className="next_link page-item">
                                  {currentPage === 1 ? null : (
                                    <Link
                                      href={`/research/r&d-labs/operational-efficiency-in-finance/pages/${prevPage}`}
                                    >
                              ...
                              {getPaginationGroup.map((item, index) => {
                                return (
                                  ...
                                  >
                                    <Link
                                      href={`/research/r&d-labs/operational-efficiency-in-finance/pages/${item}`}
                                    >
                              ...
                              {getPaginationGroup.length <= 0 ? null : (
                                <li className="next_link page-item">
                                  {currentPage >= totalPages ? null : (
                                    <Link
                                      href={`/research/r&d-labs/operational-efficiency-in-finance/pages/${nextPage}`}
                                    >
                                      <span className="page-link">
